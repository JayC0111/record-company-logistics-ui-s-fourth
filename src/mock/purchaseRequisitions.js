...
export let purchaseRequisitions = [ // 改为 let 以便增删改
    ...
    {
        id: 'pr-mock-003', // 已有，确保其状态和items
        requisition_no: 'PR-20240601-001',
        status: 'APPROVED', // 确保是 APPROVED
        requester_id: 'user-purchaser-01',
        requesterName: '采购员小王',
        request_department: '采购部',
        request_date: '2024-06-01',
        purpose: '补充《叶惠美》专辑库存',
        notes: '供应商已确认有货。审批通过，请采购。',
        items: [ // 确保有items
            { 
                id: 'prl-003-c', // 采购计划明细行ID
                purchase_requisition_id: 'pr-mock-003',
                product_id: products[0].id, // 例如：周杰伦《叶惠美》专辑
                productCode: products[0].productCode,
                productName: products[0].name,     
                specification: products[0].specification, 
                unit: products[0].unit,                 
                estimated_quantity: 50, 
                // 确保 converted_to_po_qty 或类似字段的逻辑 (如果存在) 允许此数量被选择
                // 例如，如果 estimated_quantity_remaining = estimated_quantity - converted_to_po_qty
                converted_to_po_qty: 0, // 假设尚未转换
                estimated_unit_price: parseFloat(products[0].costPrice) || 38.00, 
                suggested_supplier_id: mockSuppliers[0].id, // 例如：索尼音乐供应商
                suggestedSupplierName: mockSuppliers[0].name, 
                line_notes: '批准采购50张叶惠美'
            },
            { 
                id: 'prl-003-d',
                purchase_requisition_id: 'pr-mock-003',
                product_id: products[1].id, // 例如：林俊杰《曹操》专辑
                productCode: products[1].productCode,
                productName: products[1].name,     
                specification: products[1].specification, 
                unit: products[1].unit,                 
                estimated_quantity: 30, 
                converted_to_po_qty: 10, // 假设已部分转换了10个
                estimated_unit_price: parseFloat(products[1].costPrice) || 32.00, 
                suggested_supplier_id: mockSuppliers[1].id, // 例如：华纳唱片直供
                suggestedSupplierName: mockSuppliers[1].name, 
                line_notes: '批准采购30张曹操，已转10'
            }
        ],
        totalEstimatedAmount: (50 * (parseFloat(products[0].costPrice) || 38)) + (30 * (parseFloat(products[1].costPrice) || 32))
    },
    {
        id: 'pr-mock-004',
