...
const suppliersMock = {
    ...
    // 新增或确保 getSupplierDetail 方法存在且功能正确
    getSupplierDetail(id) {
        console.log('[Mock /suppliers.js] getSupplierDetail called for ID:', id);
        const supplier = mockSuppliers.find(s => s.id === id);
        if (supplier) {
            return Promise.resolve({
                code: 200,
                message: '获取供应商详情成功 (Mock)',
                data: JSON.parse(JSON.stringify(supplier)) // 返回副本以防意外修改
            });
        } else {
            return Promise.resolve({
                code: 404,
